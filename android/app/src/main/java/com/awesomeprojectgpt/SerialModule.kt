...
class SerialModule(
    ...
    private fun emitAirbag13Result(out: FloatArray) {
        ...
        map.putDouble("isChild", out[AirbagNative.IDX_IS_CHILD].toDouble())
        map.putDouble("isAdult", out[AirbagNative.IDX_IS_ADULT].toDouble())
        map.putDouble("childThreshold", out[AirbagNative.IDX_CHILD_THRESHOLD].toDouble())
        ...
        map.putDouble("spineProtectActive", out[AirbagNative.IDX_SPINE_ACTIVE].toDouble())
        map.putDouble("spineProtectSide", out[AirbagNative.IDX_SPINE_SIDE].toDouble())
        map.putDouble("bumpReliefActive", out[AirbagNative.IDX_BUMP_ACTIVE].toDouble())
        map.putDouble("motionSicknessActive", out[AirbagNative.IDX_MOTION_ACTIVE].toDouble())
        map.putDouble("healthReasonCode", out[AirbagNative.IDX_HEALTH_CODE].toDouble())
