...
/* ★手改（非 Simulink 生成）：无预压力热力图用的「原始矩阵」快照 ─────────────
 * 算法本体会在稳定 2 秒后记录预压力基线 pBaseB/pBaseC，之后输出的
 * backrestData1/cushionData1 都是「当前值 - 基线」。调试面板需要一份没减基线的
 * 绝对压力，所以在减基线之前把矩阵原样拷一份出来给 JNI 读。
 * 点位排布与热力图完全一致（同样是重排/镜像/250 饱和替换之后的矩阵，列优先）。
 * 注意：若哪天用 Simulink 重新生成本文件，这两个全局和 step() 里那两行 memcpy
 *      会一起丢失，链接 libairbag.so 时会直接报未定义符号（不会静默出错）。 */
...
/* Function for MATLAB Function: '<Root>/��������1' */
static void airba_calculatePressureFeatures(const real32_T matrixIn[56],
  real32_T threshold, real32_T *originalSum, real32_T *filteredSum)
{
  int32_T b;
...
/* Function for MATLAB Function: '<Root>/Ʒζϵ��1' */
static real32_T airbag_13Hz_directionOf(real32_T b_value)
{
  real32_T direction;
...
/* Function for MATLAB Function: '<Root>/���ҿ���Э��1' */
static void airbag_13Hz_applyAdaptiveGears(real32_T frame[55], real32_T
  leftWingGear, real32_T rightWingGear, real32_T lumbarGear, real32_T
  leftLegGear, real32_T rightLegGear)
{
...
void airbag_13Hz_step(void)
{
  ...
  /* MATLAB Function: '<Root>/������1' incorporates:
  ...
  /* ★手改：减预压力基线之前，把原始矩阵快照给「无预压力热力图」面板 */
  ...
  /* MATLAB Function: '<Root>/������Ħ1' incorporates:
  ...
  /* MATLAB Function: '<Root>/Ʒζϵ��1' incorporates:
  ...
  /* MATLAB Function: '<Root>/����״̬�ж�1' incorporates:
  ...
  /* MATLAB Function: '<Root>/�������ҿ����߼�1' incorporates:
   *  Inport: '<Root>/backTotalThreshold1'
   */
  ...
  /* MATLAB Function: '<Root>/������Ԥ����1' incorporates:
  ...
  /* MATLAB Function: '<Root>/���ҿ���Э��1' incorporates:
   *  DataTypeConversion: '<Root>/Data Type Conversion20'
   *  DataTypeConversion: '<Root>/Data Type Conversion25'
   *  Inport: '<Root>/adoption_frequency1'
   *  Inport: '<Root>/deflation_time1'
  ...
  /* MATLAB Function: '<Root>/�ϵ籣��Ʒζ���� 1' incorporates:
   *  MATLAB Function: '<Root>/Ʒζϵ��1'
   *  UnitDelay: '<Root>/Unit Delay2'
   */
  ...
  /* End of MATLAB Function: '<Root>/�ϵ籣��Ʒζ���� 1' */
...
{
  {
    ...
    /* SystemInitialize for MATLAB Function: '<Root>/������1' */
    airbag_13Hz_DW.noiseBaseline = 0.33F;
    airbag_13Hz_DW.noiseDev = 0.1F;

    /* SystemInitialize for MATLAB Function: '<Root>/Ʒζϵ��1' */
    for (i = 0; i < 8; i++) {
      airbag_13Hz_DW.pThresholds[i] = tmp[i];
    }

    /* End of SystemInitialize for MATLAB Function: '<Root>/Ʒζϵ��1' */
  }
}
