...
### 4.4 初始化充气流程 (init_phase 状态机)

当系统从 `OFF_SEAT` 或 `CUSHION_ONLY` 进入 `ADAPTIVE_LOCKED` 时，会启动严格顺序的初始化流程状态机（`init_phase`），确保在初始化完成前不会启动自适应控制：

1. **`waiting_recognition` (等待识别阶段)**：
   - 入座后默认进入此阶段。
   - 系统发送保持指令，等待活体检测确认。
   - **推进条件**：活体检测确认为活体（`adaptive_control_unlocked == True`）。

2. **`support_inflate` (支撑气囊初始化阶段)**：
   - 气囊1-6同时以 `gear_3`（快速充气）充气。
   - 持续 `init_inflate.cycles`（52个周期 ≈ 4秒）。
   - **推进条件**：支撑气囊充气完成。完成后检查体型三分类状态，若已完成且有品味数据，则进入 `hip_inflate`；否则直接进入 `done`。

3. **`hip_inflate` (臀托品味补充阶段)**：
   - 气囊7-8根据品味数据执行充气或放气，支撑气囊保持。
   - 基础周期数：`hip_base_cycles`（26个周期 ≈ 2秒）。
   - 品味联动：`总周期数 = 基础周期数 + 净操作次数 × 3秒 × 每秒周期数(13)`。
   - **推进条件**：臀托充放气完成。

4. **`done` (初始化完成)**：
   - 初始化流程结束，系统正式进入自适应控制阶段（Mode1/Mode2）。
