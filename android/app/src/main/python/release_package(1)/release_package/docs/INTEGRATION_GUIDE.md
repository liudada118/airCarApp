...
### 3.14. `set_manual_mode(self, enabled: bool) -> Dict`

切换自适应/手动模式。手动模式下暂停自适应控制逻辑，由外部通过 `generate_manual_command` 逐个控制气囊。

**输入参数：**

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `enabled` | `bool` | 是 | `True`=手动模式, `False`=自适应模式 |
...
### 3.15. `get_mode(self) -> str`

获取当前工作模式。

**返回值：** `str` - `'manual'` 或 `'adaptive'`

### 3.16. `generate_manual_command(self, airbag_id: int, action: str) -> Optional[list]`

生成手动控制单个气囊的协议帧。仅在手动模式下可用，同时自动更新操作计数器。

**输入参数：**

| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `airbag_id` | `int` | 是 | 气囊编号 (1-24) |
| `action` | `str` | 是 | 操作类型: `'inflate'`, `'deflate'`, `'hold'` |

**返回值：** `list[int]` - 55个10进制整数的协议帧，失败返回 `None`

### 3.17. `generate_manual_hold_all_command(self) -> list`

生成全部气囊保持的协议帧（手动模式下使用）。

**返回值：** `list[int]` - 55个10进制整数的协议帧

### 3.18. `get_manual_airbag_ops(self) -> Dict`

获取手动模式下的气囊操作计数，用于品味记录时构建置信区间。
